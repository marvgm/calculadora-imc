...
class _HomeState extends State<Home> {
  ...
  void calculate() {
    setState(() {
      ...
      double height2x = height * height;
      double value = height2x > 0.0 ? height2x : 1.0;
      double imc = weight / value;

      print(imc);

      if (imc <= 0.0) {
        _infoText = "Impossível calcular imc!";
      } else if (imc > 0.0 && imc <= 18.5) {
        _infoText = "Abaixo do peso! IMC(${imc.toStringAsPrecision(4)})";
      } else if (imc >= 18.6 && imc < 24.9) {
        _infoText = "Peso ideal. Parabéns! IMC(${imc.toStringAsPrecision(4)})";
